...
export class ShirtRack3D {
  constructor(track,slots,assetBase,onReady){
    ...
    this.rows=[...track.querySelectorAll('.rack-row')];this.renderer=this.createRenderer();
    ...
    this.rails=this.rows.map(()=>{
      const rail=new THREE.Mesh(new THREE.CylinderGeometry(RAIL_RADIUS,RAIL_RADIUS,1,32),steel);
  ...
  resize(){
    ...
    const rowWorld=this.rows.map((row,index)=>{
      ...
      const rail=this.rails[index];rail.position.set(centerX,railY,0);rail.scale.y=railLength/PX;
      const floor=this.floors[index];floor.position.set(centerX,railY-3.08,0);
      return railY;
    });
    const shortestRail=Math.min(...this.rows.map(row=>Math.max(1,row.offsetWidth-48)));
    this.railFade.value=Math.min(.2,72/shortestRail);
